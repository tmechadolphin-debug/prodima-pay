...
app.post("/api/sap/quote", verifyUser, async (req, res) => {
  try {
    ...
    // ✅ ÚNICO CAMBIO: Norma de reparto por dimensiones:
    //    Área de negocio (Dim1) = 04  -> CostingCode
    //    Departamento  (Dim2) = VTAS -> CostingCode2
    const DocumentLines = cleanLines.map((ln) => ({
      ItemCode: ln.ItemCode,
      Quantity: ln.Quantity,
      WarehouseCode: warehouseCode,

      // ✅ Norma de reparto = 04;VTAS (por dimensiones)
      CostingCode: "04",
      CostingCode2: "VTAS",
    }));
    ...
    try {
      ...
      const payloadFallback = {
        ...
        DocumentLines: cleanLines.map((ln) => ({
          ...
          // ✅ Norma de reparto = 04;VTAS (por dimensiones) TAMBIÉN EN FALLBACK
          CostingCode: "04",
          CostingCode2: "VTAS",
        })),
      };
