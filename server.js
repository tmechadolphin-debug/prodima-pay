...
/* =========================================================
   Portal Estratificación + memoria conversacional
========================================================= */
const ESTRAT_ALL_AREAS = ["CONS", "RCI"];

async function ensureEstratificacionMemoryDb() {
  if (!hasDb()) return;

  await dbQuery(`
    CREATE TABLE IF NOT EXISTS portal_user_area_permissions (
      id BIGSERIAL PRIMARY KEY,
      user_id INTEGER NOT NULL REFERENCES portal_users(id) ON DELETE CASCADE,
      module_key TEXT NOT NULL DEFAULT 'estratificacion',
  ...
  await dbQuery(`CREATE INDEX IF NOT EXISTS idx_puap_user_module ON portal_user_area_permissions(user_id, module_key)`);
  await dbQuery(`CREATE INDEX IF NOT EXISTS idx_ai_conv_module_user ON ai_conversations(module_key, user_id, updated_at DESC)`);
  await dbQuery(`CREATE INDEX IF NOT EXISTS idx_ai_msg_conv ON ai_messages(conversation_id, id)`);
  await dbQuery(`CREATE INDEX IF NOT EXISTS idx_ai_audit_module_user ON ai_audit_log(module_key, user_id, created_at DESC)`);
}
...
function normalizeEstratAreaCode(v) {
...
function uniqTextArray(arr) {
  return Array.from(new Set((Array.isArray(arr) ? arr : []).map((x) => String(x || "").trim()).filter(Boolean)));
}

function estratAllAreasAllowed(areas) {
  const set = new Set((areas || []).map((x) => normalizeEstratAreaCode(x)).filter(Boolean));
  return ESTRAT_ALL_AREAS.every((a) => set.has(a));
}

function parseEstratAreasFromPermissions(perms) {
  const list = Array.isArray(perms) ? perms.map((x) => String(x || "").trim()) : [];
  if (list.some((p) => ["*", "estratificacion:*", "estratificacion:all", "area:*"].includes(p))) {
    return ESTRAT_ALL_AREAS.slice();
  }

  const out = [];
  for (const p of list) {
    const up = p.toUpperCase();
    if (up === "CONS" || up.endsWith(":CONS")) out.push("CONS");
    if (up === "RCI" || up.endsWith(":RCI")) out.push("RCI");
  }
  return uniqTextArray(out);
}

async function getPortalUserDbRow(userId) {
  const id = Number(userId || 0);
  if (!Number.isFinite(id) || id <= 0 || !hasDb()) return null;
  const r = await dbQuery(
...
async function getEstratAllowedAreasForUser(user) {
  if (!user) return [];
  if (user?.role === "admin") return ESTRAT_ALL_AREAS.slice();

  const userId = Number(user?.id || 0);
  if (!Number.isFinite(userId) || userId <= 0 || !hasDb()) {
    return parseEstratAreasFromPermissions(user?.permissions);
  }
  ...
  try {
    const r = await dbQuery(
      `SELECT area_code
       FROM portal_user_area_permissions
       WHERE user_id=$1
         AND COALESCE(NULLIF(module_key,''), 'estratificacion') = 'estratificacion'
       ORDER BY area_code`,
      [userId]
    );
    areas = (r.rows || []).map((x) => normalizeEstratAreaCode(x.area_code)).filter(Boolean);
  } catch {}

  if (!areas.length) {
    const dbUser = await getPortalUserDbRow(userId);
    areas = parseEstratAreasFromPermissions(dbUser?.permissions_json || user?.permissions || []);
  }

  if (!areas.length) return ESTRAT_ALL_AREAS.slice();
  return uniqTextArray(areas);
}

async function getEstratPortalMe(user) {
  const dbUser = await getPortalUserDbRow(user?.id);
  const areas = await getEstratAllowedAreasForUser(dbUser || user);
  const permissions = Array.isArray(dbUser?.permissions_json)
  ...
  return {
    ...
    full_name: String(dbUser?.full_name || user?.full_name || ""),
    role: String(dbUser?.role || user?.role || "user"),
    is_active: dbUser?.is_active !== false,
    permissions,
    allowedAreas: areas,
    canSync: String(dbUser?.role || user?.role || "") === "admin",
    historyEnabled: Number(user?.id || 0) > 0,
  };
}

function resolveEstratRequestedArea(area, allowedAreas = [], role = "user") {
  const requested = String(area || "__ALL__").trim() || "__ALL__";
  const normalizedRequested = requested === "__ALL__" ? "__ALL__" : normalizeEstratAreaCode(requested);
  const allowed = uniqTextArray((allowedAreas || []).map((x) => normalizeEstratAreaCode(x)).filter(Boolean));

  if (role === "admin" || estratAllAreasAllowed(allowed)) {
    return normalizedRequested || "__ALL__";
  }

  if (!allowed.length) return normalizedRequested || "__ALL__";

  if (normalizedRequested === "__ALL__") {
    return allowed.length === 1 ? allowed[0] : "__ALL__";
  }

  if (!allowed.includes(normalizedRequested)) {
    throw new Error("No autorizado para esa área");
  }
  return normalizedRequested;
}

function estratFilterAvailableGroupsByAreas(groups = [], allowedAreas = []) {
  if (!Array.isArray(groups)) return [];
  if (!allowedAreas?.length || estratAllAreasAllowed(allowedAreas)) return groups;
  const allowedSet = new Set(allowedAreas.map((x) => normalizeEstratAreaCode(x)).filter(Boolean));
  return groups.filter((g) => {
    const area = normalizeEstratAreaCode(globalThis.inferAreaFromGroup?.(String(g || "")) || "");
    return !area || allowedSet.has(area);
  });
}

function filterEstratDashboardByAreas(data, allowedAreas = []) {
  if (!data || !allowedAreas?.length || estratAllAreasAllowed(allowedAreas)) return data;
  const allowedSet = new Set(allowedAreas.map((x) => normalizeEstratAreaCode(x)).filter(Boolean));

  const items = (Array.isArray(data.items) ? data.items : []).filter((x) =>
    allowedSet.has(normalizeEstratAreaCode(x?.area))
  );

  const groupAgg = (Array.isArray(data.groupAgg) ? data.groupAgg : []).filter((x) => {
    const inferredArea = normalizeEstratAreaCode(
      x?.area || globalThis.inferAreaFromGroup?.(String(x?.grupo || x?.label || ""))
    );
    return !inferredArea || allowedSet.has(inferredArea);
  });
  ...
  totals.gpPct = totals.revenue > 0 ? Number(((totals.gp / totals.revenue) * 100).toFixed(2)) : 0;

  return {
    ...data,
    availableGroups: estratFilterAvailableGroupsByAreas(data.availableGroups || [], allowedAreas),
    items,
    groupAgg,
    totals,
  };
...
function filterEstratRowsByAreas(rows = [], allowedAreas = []) {
  if (!allowedAreas?.length || estratAllAreasAllowed(allowedAreas)) return Array.isArray(rows) ? rows : [];
  const allowedSet = new Set(allowedAreas.map((x) => normalizeEstratAreaCode(x)).filter(Boolean));
  return (Array.isArray(rows) ? rows : []).filter((x) => allowedSet.has(normalizeEstratAreaCode(x?.area)));
}

function estratTrimText(s, max = 6000) {
  const txt = String(s || "").replace(/\s+/g, " ").trim();
  if (!txt) return "";
  if (txt.length <= max) return txt;
  return txt.slice(txt.length - max);
}

function buildEstratConversationTitle(question = "", itemCode = "", itemLabel = "") {
  const clean = String(question || "").replace(/\s+/g, " ").trim();
  const prefix = itemCode ? `${itemCode}${itemLabel ? " · " + String(itemLabel).trim() : ""} — ` : "";
  const title = (prefix + clean).trim() || "Nueva conversación";
  return title.length > 120 ? title.slice(0, 117) + "..." : title;
}

function mergeEstratSummary(prevSummary = "", question = "", answer = "") {
  const block = [
    question ? `Usuario: ${estratTrimText(question, 700)}` : "",
    answer ? `Asistente: ${estratTrimText(answer, 1400)}` : "",
  ].filter(Boolean).join("\n");
  const merged = [String(prevSummary || "").trim(), block].filter(Boolean).join("\n\n");
  return estratTrimText(merged, 6000);
}

async function getEstratConversation(conversationId, user) {
  const id = Number(conversationId || 0);
  if (!Number.isFinite(id) || id <= 0 || !hasDb()) return null;

  const params = [id];
  let sql = `
    SELECT id, module_key, user_id, title, area_code, item_code, item_label, filters_json, summary_text, created_at, updated_at
    FROM ai_conversations
    WHERE id=$1
      AND module_key='estratificacion'
  `;
...
async function createEstratConversation({ userId, title, areaCode = "", itemCode = "", itemLabel = "", filtersJson = {} }) {
  const uid = Number(userId || 0);
  if (!Number.isFinite(uid) || uid <= 0) return null;

  const r = await dbQuery(
    `INSERT INTO ai_conversations(module_key, user_id, title, area_code, item_code, item_label, filters_json, summary_text, created_at, updated_at)
     VALUES ('estratificacion', $1, $2, $3, $4, $5, $6::jsonb, '', NOW(), NOW())
     RETURNING id, module_key, user_id, title, area_code, item_code, item_label, filters_json, summary_text, created_at, updated_at`,
    [uid, String(title || "").trim(), String(areaCode || ""), String(itemCode || ""), String(itemLabel || ""), JSON.stringify(filtersJson || {})]
  );
  return r.rows?.[0] || null;
}

async function getEstratConversationMessages(conversationId, limit = 12) {
  const convId = Number(conversationId || 0);
  if (!Number.isFinite(convId) || convId <= 0 || !hasDb()) return [];
  const safeLimit = Math.max(1, Math.min(Number(limit || 12), 50));

  const r = await dbQuery(
    `SELECT id, role, content, context_json, model, created_at
     FROM ai_messages
...
async function insertEstratMessage({ conversationId, role, content, contextJson = {}, model = "", promptTokens = 0, completionTokens = 0, durationMs = 0 }) {
  const convId = Number(conversationId || 0);
  if (!Number.isFinite(convId) || convId <= 0 || !hasDb()) return;
  await dbQuery(
    `INSERT INTO ai_messages(conversation_id, role, content, context_json, model, prompt_tokens, completion_tokens, duration_ms, created_at)
...
async function updateEstratConversationAfterReply({ conversationId, areaCode = "", itemCode = "", itemLabel = "", filtersJson = {}, summaryText = "" }) {
  const convId = Number(conversationId || 0);
  if (!Number.isFinite(convId) || convId <= 0 || !hasDb()) return;
  await dbQuery(
...
async function insertEstratAuditLog({ userId = null, conversationId = null, areaCode = "", itemCode = "", question = "", answer = "", status = "ok", errorText = "" }) {
  if (!hasDb()) return;
  await dbQuery(
    `INSERT INTO ai_audit_log(module_key, user_id, conversation_id, area_code, item_code, question, answer, status, error_text, created_at)
     VALUES ('estratificacion', $1, $2, $3, $4, $5, $6, $7, $8, NOW())`,
    [
...
function buildEstratCompactContext(dashboard, itemRows = [], itemLabel = "") {
  const safeNum = (x, d = 0) => {
    const n = Number(x || 0);
    return Number.isFinite(n) ? Number(n.toFixed(d)) : 0;
  };
  ...
  const compactDashboard = {
    ...
    topGroups: groupAgg.slice(0, 20).map((x) => ({
      area: x.area || normalizeEstratAreaCode(globalThis.inferAreaFromGroup?.(String(x.grupo || x.label || "")) || ""),
      grupo: x.grupo,
      items: Number(x.items || 0),
      revenue: safeMoney(x.revenue),
      gp: safeMoney(x.gp),
...
async function openaiEstratificacionChatWithMemory({
  question,
  dashboard,
  itemRows = [],
  itemLabel = "",
  ...
  const compact = buildEstratCompactContext(dashboard, itemRows, itemLabel);

  const system = [
    "Eres un analista comercial interno senior de PRODIMA especializado en estratificación de productos, rentabilidad, clasificación ABC, niveles de inventario y riesgo comercial.",
    "Usa exclusivamente la información entregada por el sistema como fuente de verdad. No menciones formatos internos ni digas que estás usando JSON.",
    "La fuente es la base de datos sincronizada del sistema, no SAP en vivo.",
    "Respeta estrictamente los filtros activos de fecha, área, grupo, búsqueda, artículo seleccionado y permisos del usuario cuando existan.",
    "No inventes datos, no asumas ventas, stock ni márgenes que no estén presentes en el contexto.",
    "Responde siempre en español, con lenguaje claro, ejecutivo, útil y orientado a decisiones.",
    "Prioriza hallazgos concretos sobre revenue, margen bruto, % margen, clasificación ABC, ranking, concentración, stock, mínimos, máximos, disponible, comprometido y ordenado.",
    "Cuando hables de stock, distingue con precisión si el artículo está por debajo del mínimo, dentro de rango, en máximo o por encima del máximo.",
    "Si hay un artículo seleccionado, analízalo primero. Luego compáralo contra el contexto general solo si eso agrega valor.",
    "Cuando haya memoria conversacional, úsala para mantener continuidad con las preguntas previas del usuario sin contradecir el contexto actual.",
    "Si una conclusión depende del historial previo pero el contexto actual no la confirma, dilo con claridad.",
    "Cuando el usuario pida resumen, responde con estructura: resumen ejecutivo, hallazgos clave, riesgos, oportunidades y acciones sugeridas.",
    "Cuando el usuario pida un artículo específico, responde con estructura: resumen del artículo, desempeño comercial, margen, clientes, comportamiento mensual, stock y recomendación.",
    "No des respuestas genéricas. Siempre que sea posible menciona códigos, descripciones, grupos, área, montos, porcentajes y niveles de stock concretos.",
    "Tu objetivo es ayudar a ventas, gerencia y planificación a decidir qué artículos proteger, impulsar, revisar o depurar."
  ...
  const memoryNote = [
    conversationSummary ? `Resumen acumulado de la conversación:\n${conversationSummary}` : "",
    userMeta ? `Usuario autenticado:\n${JSON.stringify(userMeta)}` : "",
  ].filter(Boolean).join("\n\n");
  ...
  if (memoryNote) {
    historyInputs.push({
      role: "system",
      content: [{ type: "input_text", text: memoryNote }]
    });
  }
  ...
  for (const msg of Array.isArray(recentMessages) ? recentMessages : []) {
    ...
    const content = estratTrimText(msg?.content || "", role === "assistant" ? 1500 : 1000);
    if (!content) continue;
    historyInputs.push({
      role,
  ...
  const answer = estratExtractResponseText(data);
  if (!answer) {
    console.error("OpenAI empty output [estratificacion-memory]", {
      model,
      status: data?.status || null,
      incomplete_details: data?.incomplete_details || null,
      output_types: Array.isArray(data?.output) ? data.output.map((x) => x?.type) : [],
...
app.get("/api/portal/estratificacion/dashboard", verifyPortalAuthEstrat, async (req, res) => {
  try {
    ...
    const fromQ = String(req.query?.from || "");
    const toQ = String(req.query?.to || "");
    const areaQ = String(req.query?.area || "__ALL__");
    const grupo = String(req.query?.grupo || "__ALL__");
    const q = String(req.query?.q || "");
    ...
    const from = isISO(fromQ) ? fromQ : "2024-01-01";
    const to = isISO(toQ) ? toQ : today;
    const area = resolveEstratRequestedArea(areaQ, me.allowedAreas, me.role);

    let data = await dashboardFromDbEstratificacion({ from, to, area, grupo, q });
    data = filterEstratDashboardByAreas(data, me.allowedAreas);

    return safeJson(res, 200, { ...data, portalUser: me });
  } catch (e) {
    const msg = e?.message || String(e);
    const status = /No autorizado/i.test(msg) ? 403 : 500;
    return safeJson(res, status, { ok: false, message: msg });
  }
});
...
app.get("/api/portal/estratificacion/item-docs", verifyPortalAuthEstrat, async (req, res) => {
  try {
    ...
    let rows = await estratLoadItemDocsForAi({ itemCode, from, to, area, grupo });
    rows = filterEstratRowsByAreas(rows, me.allowedAreas);
...
app.get("/api/portal/estratificacion/conversations", verifyPortalAuthEstrat, async (req, res) => {
  try {
    ...
    if (!Number.isFinite(userId) || userId <= 0) {
      return safeJson(res, 200, { ok: true, conversations: [] });
    }

    const r = await dbQuery(
      `SELECT id, title, area_code, item_code, item_label, filters_json, summary_text, created_at, updated_at
       FROM ai_conversations
       WHERE module_key='estratificacion'
         AND user_id=$1
    ...
    const conversations = (r.rows || []).map((x) => ({
      ...
      areaCode: String(x.area_code || ""),
      itemCode: String(x.item_code || ""),
      itemLabel: String(x.item_label || ""),
      filters: x.filters_json || {},
      summaryText: String(x.summary_text || ""),
...
app.get("/api/portal/estratificacion/conversations/:id/messages", verifyPortalAuthEstrat, async (req, res) => {
  try {
    ...
    const conv = await getEstratConversation(req.params.id, req.user);
    if (!conv) return safeJson(res, 404, { ok: false, message: "Conversación no encontrada" });

    const msgs = await getEstratConversationMessages(conv.id, 200);
    return safeJson(res, 200, {
      ok: true,
      conversation: {
        id: Number(conv.id),
        title: String(conv.title || ""),
        areaCode: String(conv.area_code || ""),
        itemCode: String(conv.item_code || ""),
        itemLabel: String(conv.item_label || ""),
        filters: conv.filters_json || {},
        summaryText: String(conv.summary_text || ""),
...
app.post("/api/portal/estratificacion/ai-chat", verifyPortalAuthEstrat, async (req, res) => {
  ...
  try {
    ...
    me = await getEstratPortalMe(req.user);
    ...
    const fromQ = String(req.body?.from || "");
    const toQ = String(req.body?.to || "");
    const areaQ = String(req.body?.area || "__ALL__");
    const grupo = String(req.body?.grupo || "__ALL__");
    const q = String(req.body?.q || "");
    ...
    const from = isISO(fromQ) ? fromQ : "2025-01-01";
    const to = isISO(toQ) ? toQ : today;
    area = resolveEstratRequestedArea(areaQ, me.allowedAreas, me.role);

    if (conversationId > 0) {
      conv = await getEstratConversation(conversationId, req.user);
      if (!conv) {
        return safeJson(res, 404, { ok: false, message: "Conversación no encontrada" });
      }
    } else if (Number(me.id || 0) > 0) {
      conv = await createEstratConversation({
        userId: me.id,
        title: buildEstratConversationTitle(question, itemCode, itemLabel),
        areaCode: area === "__ALL__" ? "" : area,
        itemCode,
        itemLabel: itemLabel || itemCode,
        filtersJson: { from, to, area, grupo, q },
      });
    }

    let dashboard = await loadEstratificacionDashboardForAi({ from, to, area, grupo, q });
    dashboard = filterEstratDashboardByAreas(dashboard, me.allowedAreas);

    let itemRows = itemCode
      ? await estratLoadItemDocsForAi({ itemCode, from, to, area, grupo })
      : [];
    itemRows = filterEstratRowsByAreas(itemRows, me.allowedAreas);

    const recentMessages = conv ? await getEstratConversationMessages(conv.id, 10) : [];
    const out = await openaiEstratificacionChatWithMemory({
      question,
      dashboard,
      itemRows,
      itemLabel: itemLabel || itemCode,
      conversationSummary: conv?.summary_text || "",
      recentMessages,
      userMeta: {
    ...
    if (conv?.id) {
      ...
      await insertEstratMessage({
        ...
        contextJson,
        model: out.model,
        promptTokens: Number(out?.usage?.input_tokens || out?.usage?.prompt_tokens || 0),
        completionTokens: Number(out?.usage?.output_tokens || out?.usage?.completion_tokens || 0),
        durationMs: Number(out?.durationMs || 0),
      });

      const summaryText = mergeEstratSummary(conv.summary_text || "", question, out.answer);
      await updateEstratConversationAfterReply({
        conversationId: conv.id,
        areaCode: area === "__ALL__" ? "" : area,
        itemCode,
        itemLabel: itemLabel || itemCode,
    ...
    await insertEstratAuditLog({
      userId: Number(me.id || 0) > 0 ? me.id : null,
    ...
    return safeJson(res, 200, {
      ...
      focus: itemCode ? { itemCode, itemLabel: itemLabel || itemCode } : null,
      conversationId: conv?.id || null,
      conversationTitle: String(conv?.title || buildEstratConversationTitle(question, itemCode, itemLabel)),
      historyEnabled: Boolean(conv?.id),
      portalUser: me,
    });
  } catch (e) {
    try {
...
app.get("/api/portal/admin/estratificacion/users/:id/areas", verifyPortalAdminEstrat, async (req, res) => {
  try {
    ...
    return safeJson(res, 200, {
      ok: true,
      userId: id,
      areas: uniqTextArray((r.rows || []).map((x) => normalizeEstratAreaCode(x.area_code)).filter(Boolean)),
    });
  } catch (e) {
    return safeJson(res, 500, { ok: false, message: e?.message || String(e) });
  }
...
app.put("/api/portal/admin/estratificacion/users/:id/areas", verifyPortalAdminEstrat, async (req, res) => {
  ...
  try {
    ...
    const id = Number(req.params.id || 0);
    const areas = uniqTextArray((Array.isArray(req.body?.areas) ? req.body.areas : []).map((x) => normalizeEstratAreaCode(x)).filter(Boolean));

    if (!Number.isFinite(id) || id <= 0) return safeJson(res, 400, { ok: false, message: "ID inválido" });
    if (!areas.length) return safeJson(res, 400, { ok: false, message: "Debes enviar al menos un área" });

    const userCheck = await client.query(`SELECT id FROM portal_users WHERE id=$1 LIMIT 1`, [id]);
    if (!userCheck.rowCount) return safeJson(res, 404, { ok: false, message: "Usuario no encontrado" });

    await client.query("BEGIN");
    await client.query(
      `DELETE FROM portal_user_area_permissions
       WHERE user_id=$1
         AND COALESCE(NULLIF(module_key,''), 'estratificacion')='estratificacion'`,
      [id]
    );
    ...
    for (const area of areas) {
      await client.query(
        `INSERT INTO portal_user_area_permissions(user_id, module_key, area_code, created_at)
         VALUES ($1, 'estratificacion', $2, NOW())
    ...
    return safeJson(res, 200, { ok: true, userId: id, areas });
  } catch (e) {
