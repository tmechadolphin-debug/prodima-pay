...
async function slLogin() {
  ...
  // Tomamos SOLO B1SESSION y ROUTEID si vienen
  const cookies = [];
...
async function slFetch(path) {
  ...
  if (!r.ok) {
    // si expiró sesión, reintenta una vez
...
async function sapGetByDocEntry(entity, docEntry, select) {
  ...
  // ✅ NO $expand. DocumentLines viene dentro del documento completo.
  return slFetch(path);
}
...
/* =========================================================
   ✅ TRACE logic + cache
========================================================= */
const TRACE_CACHE = new Map(); // key: quoteDocEntry or quoteDocNum
...
async function traceQuote(quoteDocNum, fromOverride, toOverride) {
  ...
  // Orders candidatos por CardCode + rango
...
app.get("/api/admin/quotes", verifyAdmin, async (req, res) => {
  try {
    ...
    // paginación: preferimos top/skip; si viene limit lo usamos como top fallback
    ...
    // Si no pasan fechas, tomamos últimos 30 días (para evitar cargas gigantes)
    const now = new Date();
    ...
    // Traemos Quotations en rango (sin filtros de texto para evitar errores OData por funciones)
    ...
    // Normalizamos al formato que usa tu HTML
    ...
    // Filtrado user/client en backend (sin romper OData)
    if (userFilter) {
    ...
    // ✅ Enriquecer entregado SOLO para lo que se devuelve (típico: 20 por página)
    if (withDelivered && out.length) {
      // Concurrency muy baja para no tumbar Service Layer
      const CONC = 2;
      let idx = 0;
      ...
      async function worker() {
        while (idx < out.length) {
          const i = idx++;
          const q = out[i];
          // Si está cancelada, no traceamos (entregado queda 0)
...
app.get("/api/admin/dashboard", verifyAdmin, async (req, res) => {
  // Tu HTML recalcula el dashboard; este endpoint solo debe existir y responder ok
  // (Si luego quieres mover el cálculo al server, lo hacemos, pero aquí garantizamos “sin errores”).
  safeJson(res, 200, { ok: true });
});
